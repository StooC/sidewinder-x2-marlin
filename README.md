...
| ~~Heater Minimum Temperature~~   | 5 C                | ~~-5 C~~                      | ~~HEATER_0_MINTEMP etc~~                             | ~~5 C is a bad default because it is a reasonable ambient temperature in a garage~~ *Prefer to leave this at the Artillery default*                         |
...
| ~~Possible to detach servo~~     | Off                | ~~On~~                        | ~~SERVO_DETACH_GCODE~~                               | ~~Add ability to send `M282` for lower power idle state~~ *Prefer to leave this at the Artillery default*                                                   |
| Kickstarting fans            | Off                | 100ms                     | FAN_KICKSTART_TIME                               | Allows the fan to spin up reliably                                                                       |
| ~~Auto Aligning Steppers~~       | Off                | ~~On~~                        | ~~Z_STEPPER_AUTO_ALIGN~~                             | ~~There are 2 Z axis steppers TBD check if this is needed~~ *Prefer to leave this at the Artillery default if unsure if needed*                                                  |
...
| ~~Lower Case G-Code~~            | Off                | ~~On~~                        | ~~GCODE_CASE_INSENSITIVE~~                           | ~~Why not~~ *no reason to add the small overhead and GCODE is established as upper case only in slicers*                                                                                                  |
